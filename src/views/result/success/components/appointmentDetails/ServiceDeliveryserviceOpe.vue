<template>
  ...
  >
    <a-alert
      :type="form.supplierId !== '' ? 'info' : 'error'"
      style="margin-bottom: 20px"
      >{{
        form.supplierId !== '' ? '配送服务' : '您暂时没绑定供应商！请联系管理员'
      }}</a-alert
    >
    ...
    <a-form-item field="price" label="报价" :rules="[{ required: true }]">
      <a-input-number
        v-model="form.price"
        :style="{ width: '100%' }"
        placeholder="尚未报价"
...
import { getDeliveryDetails } from '@/api/appointment';
...
export default {
  ...
  setup(props) {
    ...
    const measurementDetails = async (uid: any) => {
      const res = await getDeliveryDetails({ id: uid });
      form.value = res.data;
      console.log(res);
    };
